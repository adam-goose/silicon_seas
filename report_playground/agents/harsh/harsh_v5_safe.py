...
class HarshCompany_Latest(TradingCompany):
    """
    ...
    PROFIT_MARGIN = 0.08     # desired markup over cost
    UNDERCUT_FACTOR = 0.06     # shave a bit off the margin
    RANDOM_BID_SPREAD = 0.03   # +/- 2% noise on bids

    # -------- Safety / selectivity --------
    MAX_BIDS_PER_AUCTION = 6    # max trades to bid on in one auction
    MIN_WINDOW_LENGTH = 250   # ignore trades with very tight time windows
    MAX_TRADE_DISTANCE = 65000  # ignore very long-haul trades

    # -------- Multi-start scheduling --------
    NUM_STARTS = 8              # how many random orderings to try (plus the original)
    START_SHUFFLE_SEED = 42     # base seed to keep behaviour reproducible-ish

    # -------- Future positioning influence --------
    FUTURE_POSITIONING_STRENGTH = 0.15
...
#     return HarshSafeCompany(name=name)
# -----------------------------------------------------
COMPANY_CLASS = HarshCompany_Latest